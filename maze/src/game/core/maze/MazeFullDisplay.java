...
public class MazeFullDisplay extends JPanel {
   ...
   public void paint(Graphics g) {
      ...
      for (Coordinate c : viewMap) {
          ...
          if (mazeCopy.getRoom(c).contains("The Darkness")) {
              g.drawString("DARKNESS", x+5, y_item+=10);
          }
          if (mazeCopy.getRoom(c).contains("Oily Rag")) {
              g.drawString("Rag", x+5, y_item+=10);
          }
          if (mazeCopy.getRoom(c).contains("Wooden Stick")) {
              g.drawString("Stick", x+5, y_item+=10);
          }
          if (mazeCopy.getRoom(c).contains("Matches")) {
              g.drawString("Matches", x+5, y_item+=10);
          }
          if (mazeCopy.getRoom(c).contains("Locked Gate")) {
              g.drawString("Gate", x+5, y_item+=10);
          }
          if (mazeCopy.getRoom(c).contains("Plain Key")) {
              g.drawString("Key", x+5, y_item+=10);
          }
          if (mazeCopy.getRoom(c).contains("Red Door")) {
              g.drawString("Red Door", x+5, y_item+=10);
          }
          if (mazeCopy.getRoom(c).contains("Red Key")) {
              g.drawString("Red Key", x+5, y_item+=10);
          }
          if (mazeCopy.getRoom(c).contains("Blue Door")) {
              g.drawString("Blue Door", x+5, y_item+=10);
          }
          if (mazeCopy.getRoom(c).contains("Blue Key")) {
              g.drawString("Blue Key", x+5, y_item+=10);
          }
          if (mazeCopy.getRoom(c).contains("Purple Door")) {
          ...
          for (Coordinate neighbor : viewNeighbors) {
              ...
              if (!mazeCopy.viewNeighborsOf(neighbor).contains(c)) {
      ...
      for (Gate gate : mazeCopy.map().getGates()) {
       ...
       public static void main(String[] args) {
           ...
           mazeCopy = MazeFactory.buildMaze(openSet, 30, center);

           Statistics.initialize();

           //Maze mazeCopy = MazeFactory.buildMaze(30);

           //mazeCopy.setExit(new Coordinate(1,1));
           log("Exit location => " + mazeCopy.exit());
           log("Center location => " + mazeCopy.center());
           log("Final size: " + mazeCopy.size());

           mazeCopy.populateRooms();

           //AStar.discover(mazeCopy);
           ...
           Player you = new Player(mazeCopy);
           you.setName(name);

           util.Print.print("\nWelcome to the Maze, " + you.name() + "!");

           util.Print.print(mazeCopy.map().viewTwoLinkNodes());
